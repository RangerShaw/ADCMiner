...
public class BufferedEvidenceSetBuilder {
    ...
    private IEvidenceSet invertEvidenceUsingCorrectionMasks() {
        ...
        int currLen = currentIntervalLength();
        for (long i = 0; i < currLen; i += bufferLength) {
            bufferIntervals.add(Interval.toInterval(i, Math.min(currLen, i + bufferLength), Interval.INTERVAL_OPEN_END));
        }

        /** transposition **/
        for (Interval<Long> interval : bufferIntervals) {
            int begin = interval.getBegin().intValue();
            int end = interval.getEnd().intValue(), bLen = end-begin;
